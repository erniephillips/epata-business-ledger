...
public static class TestProgram
{
    public static async Task Main()
    {
        ...
        AssertRouteTableHasNoDuplicateHttpContracts(factory);
        await AssertEndpointGroupReadsAsync(client, dbPath);
        await AssertPrivacySafeFreshDatabaseAsync(client);
        await AssertConfigAndAiStatusAsync(client);
        await AssertGenericRouteListsAsync(client);
        await AssertCrudPayloadBindingSafetyAsync(client);
        ...
        await AssertReceivableIdentitySafetyAsync(client, concurrentClient);
        await AssertInvoiceDocumentRestoreConcurrencySafetyAsync(client, concurrentClient);
        await AssertInvoiceDocumentCaseIdentitySafetyAsync(client);
        await AssertLegacyImportSkipsArchivedInvoiceAsync(client);
        await AssertSettingsCannotBeArchivedAsync(client);
        await AssertAutomaticInvoiceDocumentIntakeAsync(client, root);
        await AssertInvoiceDocumentIntakeConcurrencySafetyAsync(client, concurrentClient, root);
        ...
        await AssertAiEstimateDraftExtractsDocumentPacketAsync(client);
        await AssertLocalAiTextCompletionUsesRequestedTokenBudgetAsync(root);

        Console.WriteLine(JsonSerializer.Serialize(new
        {
            WebApplicationFactoryRound100 = "pass",
            RouteTableUniqueness = "pass",
            CrudPayloadBindingSafety = "pass",
            ...
            AiEstimateDocumentPacketExtraction = "pass",
    ...
    static async Task AssertPrivacySafeFreshDatabaseAsync(HttpClient client)
    {
        foreach (var endpoint in new[]
        {
            "/api/parties?includeArchived=true",
            "/api/sales?includeArchived=true",
            "/api/customer-jobs?includeArchived=true",
            "/api/receivable-invoices?includeArchived=true",
            "/api/audit-documents?includeArchived=true",
            "/api/assets?includeArchived=true",
            "/api/makerworld-rewards?includeArchived=true"
        })
        {
            var records = await GetJsonArrayAsync(client, endpoint);
            AssertEqual("0", records.Length.ToString(CultureInfo.InvariantCulture), $"privacy-safe first run {endpoint}");
        }
    }

    static void AssertRouteTableHasNoDuplicateHttpContracts(EpataFactory factory)
    {
        var contracts = factory.Services
        ...
        var duplicates = contracts
            .GroupBy(contract => contract, StringComparer.OrdinalIgnoreCase)
            .Where(group => group.Count() > 1)
            .Select(group => $"{group.Key} ({group.Count()} registrations)")
            .OrderBy(contract => contract, StringComparer.OrdinalIgnoreCase)
            .ToArray();

        AssertTrue(duplicates.Length == 0, $"Duplicate HTTP route contracts were registered: {string.Join(", ", duplicates)}");
    }

    static async Task AssertCrudPayloadBindingSafetyAsync(HttpClient client)
    {
        using (var malformed = await client.PostAsync(
                   "/api/products",
                   new StringContent("{", Encoding.UTF8, "application/json")))
        {
            AssertStatus(HttpStatusCode.BadRequest, malformed.StatusCode, "malformed generic CRUD JSON");
        }

        using (var scalar = await client.PostAsync(
                   "/api/products",
                   new StringContent("\"not an object\"", Encoding.UTF8, "application/json")))
        {
            AssertStatus(HttpStatusCode.BadRequest, scalar.StatusCode, "scalar generic CRUD JSON");
            AssertContains("valid Product JSON object", await scalar.Content.ReadAsStringAsync(), "scalar generic CRUD response");
        }

        var product = await PostJsonAsync(client, "/api/products", new
        {
            name = "WAF payload binding fixture",
            sku = "WAF-BIND-1",
            category = "Test",
            notes = "original"
        });
        var id = product.GetProperty("id").GetInt32();
        using (var scalarUpdate = await client.PutAsync(
                   $"/api/products/{id}",
                   new StringContent("42", Encoding.UTF8, "application/json")))
        {
            AssertStatus(HttpStatusCode.BadRequest, scalarUpdate.StatusCode, "scalar generic CRUD update JSON");
        }

        var unchanged = await GetJsonAsync(client, $"/api/products/{id}");
        AssertEqual("original", unchanged.GetProperty("notes").GetString(), "invalid CRUD update leaves the row unchanged");
    }
    ...
    static async Task AssertMarketplaceRestoreCollisionSafetyAsync(HttpClient client, HttpClient concurrentClient)
    {
        ...
        using (var archiveFirst = await client.DeleteAsync($"/api/sales/{firstSaleId}"))
        {
            AssertStatus(HttpStatusCode.NoContent, archiveFirst.StatusCode, "archive first normalized marketplace Sale fixture");
        }
        ...
        using (var archiveSecond = await client.DeleteAsync($"/api/sales/{secondSaleId}"))
        {
            AssertStatus(HttpStatusCode.NoContent, archiveSecond.StatusCode, "archive second normalized marketplace Sale fixture");
        }
        ...
        try
        {
            ...
            using var blockedCreate = await client.PostAsJsonAsync("/api/sales", new
            {
                saleDate = "2099-09-11",
                platform = "et_sy",
                orderNumber = "WAF RESTORE SALE 2099",
                customerName = "WAF Duplicate Create",
                productName = "Duplicate marketplace create",
                quantity = 1,
                itemSales = 14,
                customerPaid = 14,
                status = "Paid",
            ...
            AssertStatus(HttpStatusCode.Conflict, blockedCreate.StatusCode, "generic Sale create rejects normalized active duplicate");

            var unrelatedSale = await PostJsonAsync(client, "/api/sales", new
            {
                saleDate = "2099-09-11",
                platform = "Etsy",
                orderNumber = "WAF-RESTORE-SALE-UNRELATED",
                customerName = "WAF Update Collision",
                productName = "Marketplace update fixture",
                quantity = 1,
                itemSales = 16,
                customerPaid = 16,
                status = "Paid",
            ...
            using var blockedUpdate = await client.PutAsJsonAsync($"/api/sales/{unrelatedSale.GetProperty("id").GetInt32()}", new
            {
                saleDate = "2099-09-11",
                platform = "ET SY",
                orderNumber = "waf restore_sale #2099",
            ...
            AssertStatus(HttpStatusCode.Conflict, blockedUpdate.StatusCode, "generic Sale update rejects normalized active duplicate");
        }
        finally
        {
        ...
        const string concurrentOrder = "WAF-GENERIC-SALE-RACE-2099";
        var createBarrier = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        var concurrentCreates = new[]
        {
            CreateSaleAfterBarrierAsync(client, createBarrier.Task, "Etsy", concurrentOrder),
            CreateSaleAfterBarrierAsync(concurrentClient, createBarrier.Task, "ET SY", "waf generic_sale race #2099")
        };
        createBarrier.SetResult(true);
        var createResponses = await Task.WhenAll(concurrentCreates);
        try
        {
            AssertEqual("1", createResponses.Count(response => response.StatusCode == HttpStatusCode.Created).ToString(CultureInfo.InvariantCulture), "simultaneous generic Sale create winner count");
            AssertEqual("1", createResponses.Count(response => response.StatusCode == HttpStatusCode.Conflict).ToString(CultureInfo.InvariantCulture), "simultaneous generic Sale create conflict count");
        }
        finally
        {
    ...
    static async Task<HttpResponseMessage> CreateSaleAfterBarrierAsync(
        HttpClient client,
        Task barrier,
        string platform,
        string orderNumber)
    {
        await barrier;
        return await client.PostAsJsonAsync("/api/sales", new
        {
            saleDate = "2099-09-12",
            platform,
            orderNumber,
            customerName = "WAF Concurrent Sale",
            productName = "Concurrent marketplace fixture",
            quantity = 1,
            itemSales = 18,
            customerPaid = 18,
            status = "Paid",
    ...
    static async Task AssertReceivableIdentitySafetyAsync(HttpClient client, HttpClient concurrentClient)
    {
        const string invoiceNumber = "INV-2099-MANUAL-AR-RACE";
        var barrier = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        var creates = new[]
        {
            CreateReceivableAfterBarrierAsync(client, barrier.Task, invoiceNumber, "manual-race-a.pdf"),
            CreateReceivableAfterBarrierAsync(concurrentClient, barrier.Task, "inv-2099-manual-ar-race", "manual-race-b.pdf")
        };
        ...
        try
        {
            AssertEqual("1", responses.Count(response => response.StatusCode == HttpStatusCode.Created).ToString(CultureInfo.InvariantCulture), "simultaneous manual AR create winner count");
            AssertEqual("1", responses.Count(response => response.StatusCode == HttpStatusCode.Conflict).ToString(CultureInfo.InvariantCulture), "simultaneous manual AR create conflict count");
            archivedId = (await ReadJsonAsync(responses.Single(response => response.StatusCode == HttpStatusCode.Created)))
                .GetProperty("id")
                .GetInt32();
        }
        finally
        ...
        using (var archive = await client.DeleteAsync($"/api/receivable-invoices/{archivedId}"))
        {
            AssertStatus(HttpStatusCode.NoContent, archive.StatusCode, "archive manual AR identity winner");
        }

        var replacement = await PostJsonAsync(
            client,
            "/api/receivable-invoices",
            ManualReceivablePayload("inv-2099-manual-ar-race", "manual-race-replacement.pdf", "Paid", 50));
        var replacementId = replacement.GetProperty("id").GetInt32();

        using (var blockedRestore = await client.PostAsJsonAsync($"/api/receivable-invoices/{archivedId}/restore", new { }))
        {
            AssertStatus(HttpStatusCode.Conflict, blockedRestore.StatusCode, "manual AR restore rejects active case-insensitive identity collision");
            var conflict = await ReadJsonAsync(blockedRestore);
            AssertEqual(replacementId.ToString(CultureInfo.InvariantCulture), conflict.GetProperty("existingReceivableInvoiceId").GetInt32().ToString(CultureInfo.InvariantCulture), "manual AR restore conflict identifies active receivable");
        }

        using (var blockedInvoiceDocument = await client.PostAsJsonAsync(
            "/api/invoice-documents",
            NewDocumentPayload("INVOICE", "Paid", invoiceNumber, 30)))
        {
            AssertStatus(HttpStatusCode.BadRequest, blockedInvoiceDocument.StatusCode, "Invoice Record create rejects active manual AR identity collision");
            AssertContains("active receivable", await blockedInvoiceDocument.Content.ReadAsStringAsync(), "Invoice Record collision explains existing manual AR ownership");
        }

        var unrelated = await PostJsonAsync(
            client,
            "/api/receivable-invoices",
            ManualReceivablePayload("WAF-MANUAL-AR-UNRELATED-2099", "manual-unrelated.pdf", "Sent", 0));
        using (var blockedUpdate = await client.PutAsJsonAsync(
            $"/api/receivable-invoices/{unrelated.GetProperty("id").GetInt32()}",
            ManualReceivablePayload(invoiceNumber, "manual-update-collision.pdf", "Sent", 0)))
        {
            AssertStatus(HttpStatusCode.Conflict, blockedUpdate.StatusCode, "manual AR update rejects active case-insensitive identity collision");
        }

        var archivedUpdatePayload = new
        {
            invoiceNumber,
            ...
            needsReview = false,
            sourceProof = "manual-race-a.pdf",
            isArchived = false
        };
        using (var blockedArchiveBypass = await client.PutAsJsonAsync($"/api/receivable-invoices/{archivedId}", archivedUpdatePayload))
        {
            AssertStatus(HttpStatusCode.OK, blockedArchiveBypass.StatusCode, "manual AR update leaves archive state to restore endpoint");
            AssertTrue((await ReadJsonAsync(blockedArchiveBypass)).GetProperty("isArchived").GetBoolean(), "generic PUT cannot restore archived manual AR");
        }

        var archivedSummaryRows = await GetJsonArrayAsync(client, "/api/invoice-documents?includeArchived=true&q=INV-2099-MANUAL-AR-RACE");
        AssertTrue(archivedSummaryRows.Any(row =>
            row.GetProperty("sourceKind").GetString() == "receivable"
            && row.GetProperty("id").GetInt32() == archivedId
            && row.GetProperty("isArchived").GetBoolean()),
            "Invoice Records include archived manual AR with archive state");
        var activeSummaryRows = await GetJsonArrayAsync(client, "/api/invoice-documents?q=INV-2099-MANUAL-AR-RACE");
        AssertTrue(activeSummaryRows.All(row => row.GetProperty("id").GetInt32() != archivedId), "Invoice Records default list excludes archived manual AR");
    }

    static async Task<HttpResponseMessage> CreateReceivableAfterBarrierAsync(
        HttpClient client,
        Task barrier,
        ...
        string sourceProof)
    {
        ...
        return await client.PostAsJsonAsync(
            "/api/receivable-invoices",
            ManualReceivablePayload(invoiceNumber, sourceProof, "Paid", 50));
    }

    static async Task AssertInvoiceDocumentCaseIdentitySafetyAsync(HttpClient client)
    {
        const string documentNumber = "INV-2099-CASE-IDENTITY";
        var document = await PostJsonAsync(
            client,
            "/api/invoice-documents",
            NewDocumentPayload("INVOICE", "Paid", documentNumber, 30));
        var documentId = document.GetProperty("id").GetInt32();

        using (var caseOnlySave = await client.PutAsJsonAsync(
            $"/api/invoice-documents/{documentId}",
            NewDocumentPayload("INVOICE", "Paid", documentNumber.ToLowerInvariant(), 30)))
        {
            AssertStatus(HttpStatusCode.OK, caseOnlySave.StatusCode, "case-only Invoice Record save");
            AssertEqual(documentNumber, (await ReadJsonAsync(caseOnlySave)).GetProperty("docNumber").GetString(), "case-only save preserves existing live document identity");
        }

        var generatedReceivables = (await GetJsonArrayAsync(client, "/api/receivable-invoices?includeArchived=true"))
            .Where(row => string.Equals(row.GetProperty("invoiceNumber").GetString(), documentNumber, StringComparison.OrdinalIgnoreCase))
            .ToArray();
        var generatedSales = (await GetJsonArrayAsync(client, "/api/sales?includeArchived=true"))
            .Where(row => string.Equals(row.GetProperty("invoiceNumber").GetString(), documentNumber, StringComparison.OrdinalIgnoreCase))
            .ToArray();
        AssertEqual("1", generatedReceivables.Length.ToString(CultureInfo.InvariantCulture), "case-only save keeps one generated AR row");
        AssertEqual("1", generatedSales.Length.ToString(CultureInfo.InvariantCulture), "case-only save keeps one generated Sale row");

        using (var archive = await client.DeleteAsync($"/api/invoice-documents/{documentId}"))
        {
            AssertStatus(HttpStatusCode.OK, archive.StatusCode, "archive case-insensitive Invoice Record fixture");
        }
        using (var blockedReuse = await client.PostAsJsonAsync(
            "/api/invoice-documents",
            NewDocumentPayload("INVOICE", "Paid", documentNumber.ToLowerInvariant(), 30)))
        {
            AssertStatus(HttpStatusCode.BadRequest, blockedReuse.StatusCode, "archived Invoice Record reserves number case-insensitively");
        }
        await PostJsonAsync(client, $"/api/invoice-documents/{documentId}/restore", new { });

        generatedReceivables = (await GetJsonArrayAsync(client, "/api/receivable-invoices?includeArchived=true"))
            .Where(row => string.Equals(row.GetProperty("invoiceNumber").GetString(), documentNumber, StringComparison.OrdinalIgnoreCase))
            .ToArray();
        generatedSales = (await GetJsonArrayAsync(client, "/api/sales?includeArchived=true"))
            .Where(row => string.Equals(row.GetProperty("invoiceNumber").GetString(), documentNumber, StringComparison.OrdinalIgnoreCase))
            .ToArray();
        AssertEqual("1", generatedReceivables.Length.ToString(CultureInfo.InvariantCulture), "case-insensitive restore keeps one generated AR row");
        AssertEqual("1", generatedSales.Length.ToString(CultureInfo.InvariantCulture), "case-insensitive restore keeps one generated Sale row");
    }

    static async Task AssertSettingsCannotBeArchivedAsync(HttpClient client)
    {
        var setting = await PostJsonAsync(client, "/api/settings", new
        {
            key = "WAF:ArchiveSafety",
            value = "active",
            notes = "Settings archive safety fixture"
        });
        var settingId = setting.GetProperty("id").GetInt32();

        using (var archive = await client.DeleteAsync($"/api/settings/{settingId}"))
        {
            AssertStatus(HttpStatusCode.Conflict, archive.StatusCode, "AppSetting archive is explicitly disabled");
            AssertContains("cannot be archived", await archive.Content.ReadAsStringAsync(), "AppSetting archive response explains live configuration semantics");
        }
        using (var putArchiveBypass = await client.PutAsJsonAsync($"/api/settings/{settingId}", new
        {
            key = "WAF:ArchiveSafety",
            value = "updated",
            notes = "Settings archive safety fixture",
        ...
        {
            AssertStatus(HttpStatusCode.OK, putArchiveBypass.StatusCode, "AppSetting update ignores archive-state injection");
            AssertTrue(!(await ReadJsonAsync(putArchiveBypass)).GetProperty("isArchived").GetBoolean(), "AppSetting remains active after archive-state injection");
        }
        using (var restore = await client.PostAsJsonAsync($"/api/settings/{settingId}/restore", new { }))
        {
            AssertStatus(HttpStatusCode.Conflict, restore.StatusCode, "AppSetting restore is explicitly disabled");
            AssertContains("do not support archive or restore", await restore.Content.ReadAsStringAsync(), "AppSetting restore response explains live configuration semantics");
        }
    }

    static async Task AssertLegacyImportSkipsArchivedInvoiceAsync(HttpClient client)
    {
        const string documentNumber = LegacyInvoiceAppHandler.DocumentNumber;
        var document = await PostJsonAsync(
            client,
            "/api/invoice-documents",
            NewDocumentPayload("INVOICE", "Paid", documentNumber, 30));
        ...
        using (var archive = await client.DeleteAsync($"/api/invoice-documents/{documentId}"))
        {
            AssertStatus(HttpStatusCode.OK, archive.StatusCode, "archive legacy import source fixture");
        }

        var import = await PostJsonAsync(client, "/api/invoice-documents/import-from-legacy", new { });
        AssertTrue(import.GetProperty("success").GetBoolean(), "legacy import request succeeds while skipping archived source identity");
        AssertEqual("0", import.GetProperty("imported").GetInt32().ToString(CultureInfo.InvariantCulture), "legacy import does not update archived source");
        AssertEqual("1", import.GetProperty("skippedArchivedDocuments").GetInt32().ToString(CultureInfo.InvariantCulture), "legacy import reports archived source skip");

        var archivedDocument = await GetJsonAsync(client, $"/api/invoice-documents/{documentId}");
        AssertTrue(archivedDocument.GetProperty("isArchived").GetBoolean(), "legacy import leaves source Invoice Record archived");
        AssertEqual("WAF Customer", archivedDocument.GetProperty("customerName").GetString(), "legacy import does not overwrite archived source fields");
        ...
        var generatedSales = (await GetJsonArrayAsync(client, "/api/sales?includeArchived=true"))
            .Where(row => string.Equals(row.GetProperty("invoiceNumber").GetString(), documentNumber, StringComparison.OrdinalIgnoreCase))
            .ToArray();
        AssertEqual("1", generatedReceivables.Length.ToString(CultureInfo.InvariantCulture), "legacy import keeps one generated AR for archived source");
        AssertEqual("1", generatedSales.Length.ToString(CultureInfo.InvariantCulture), "legacy import keeps one generated Sale for archived source");
        AssertTrue(generatedReceivables[0].GetProperty("isArchived").GetBoolean(), "legacy import leaves generated AR archived");
        AssertTrue(generatedSales[0].GetProperty("isArchived").GetBoolean(), "legacy import leaves generated Sale archived");
    }
    ...
    static async Task AssertInvoiceDocumentRestoreConcurrencySafetyAsync(HttpClient client, HttpClient concurrentClient)
    {
        const string documentNumber = "INV-2099-RESTORE-RACE";
        var document = await PostJsonAsync(
            client,
            "/api/documents",
            NewDocumentPayload("INVOICE", "Paid", documentNumber, 30m));
        var documentId = document.GetProperty("id").GetInt32();

        using (var archive = await client.DeleteAsync($"/api/documents/{documentId}"))
        {
            AssertStatus(HttpStatusCode.OK, archive.StatusCode, "archive invoice restore concurrency fixture");
        }

        var barrier = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        var restores = new[]
        {
            RestoreAfterBarrierAsync(client, barrier.Task, $"/api/documents/{documentId}/restore"),
            RestoreAfterBarrierAsync(concurrentClient, barrier.Task, $"/api/invoice-documents/{documentId}/restore")
        };
        barrier.SetResult(true);
        var responses = await Task.WhenAll(restores);
        try
        {
            AssertEqual("2", responses.Count(response => response.StatusCode == HttpStatusCode.OK).ToString(CultureInfo.InvariantCulture), "simultaneous invoice restore idempotent success count");

            var restored = await GetJsonAsync(client, $"/api/documents/{documentId}");
            AssertTrue(!restored.GetProperty("isArchived").GetBoolean(), "simultaneous invoice restore leaves the document active");

            var sourceProof = $"Unified invoice {documentNumber}";
            var matchingReceivables = (await GetJsonArrayAsync(client, "/api/receivable-invoices?includeArchived=true"))
                .Where(invoice => string.Equals(invoice.GetProperty("invoiceNumber").GetString(), documentNumber, StringComparison.Ordinal)
                    && string.Equals(invoice.GetProperty("sourceProof").GetString(), sourceProof, StringComparison.Ordinal))
                .ToArray();
            AssertEqual("1", matchingReceivables.Length.ToString(CultureInfo.InvariantCulture), "simultaneous invoice restore keeps one generated receivable");
            AssertTrue(!matchingReceivables[0].GetProperty("isArchived").GetBoolean(), "simultaneous invoice restore reactivates the generated receivable");

            var matchingSales = (await GetJsonArrayAsync(client, "/api/sales?includeArchived=true"))
                .Where(sale => string.Equals(sale.GetProperty("invoiceNumber").GetString(), documentNumber, StringComparison.Ordinal)
                    && string.Equals(sale.GetProperty("sourceProof").GetString(), sourceProof, StringComparison.Ordinal))
                .ToArray();
            AssertEqual("1", matchingSales.Length.ToString(CultureInfo.InvariantCulture), "simultaneous invoice restore keeps one generated Sale");
            AssertTrue(!matchingSales[0].GetProperty("isArchived").GetBoolean(), "simultaneous invoice restore reactivates the generated Sale");
        }
        finally
        {
            foreach (var response in responses)
    ...
    static async Task AssertCrudAndDocumentWorkflowsAsync(HttpClient client)
    {
        ...
        using (var archiveMovedJob = await client.DeleteAsync($"/api/customer-jobs/{movedQueueJobId}"))
        {
            AssertStatus(HttpStatusCode.NoContent, archiveMovedJob.StatusCode, "archive job while linked queue remains active");
        }
        using (var completeQueueWhileJobArchived = await client.PutAsJsonAsync($"/api/printer-queue-items/{queueItemId}", new
        {
            queueDate = "2026-06-23",
            priority = "Normal",
            status = "Completed",
        ...
        {
            AssertStatus(HttpStatusCode.OK, completeQueueWhileJobArchived.StatusCode, "complete queue while linked job is archived");
        }
        AssertEqual("In Progress", (await GetJsonAsync(client, $"/api/customer-jobs/{movedQueueJobId}")).GetProperty("status").GetString(), "archived job keeps its prior status until restore");
        await PostJsonAsync(client, $"/api/customer-jobs/{movedQueueJobId}/restore", new { });
        AssertEqual("Completed", (await GetJsonAsync(client, $"/api/customer-jobs/{movedQueueJobId}")).GetProperty("status").GetString(), "restoring job recomputes status from active queue items");
    }
    ...
    static async Task AssertInvoiceDocumentIntakeConcurrencySafetyAsync(
        HttpClient client,
        HttpClient concurrentClient,
        string root)
    {
        const string documentNumber = "INV-2099-8017";
        const string firstFileName = "WAF-concurrent-invoice-intake-a.pdf";
        const string secondFileName = "WAF-concurrent-invoice-intake-b.pdf";
        var commonLines = new[]
        {
            "EPATA 3D PRINTS",
            "INVOICE",
            ...
            "Date August 20, 2099",
            "Due Date September 3, 2099",
            "Prepared For Concurrent Intake Customer",
            "SENT",
            "BILL TO PROJECT DETAILS PRICING SUMMARY",
            "Concurrent Intake Customer",
            "Project Name: Concurrent Intake Fixture",
            ...
            "INVOICE BREAKDOWN",
            "# DESCRIPTION CALCULATION / DETAILS QTY RATE AMOUNT",
            "1 Test part Concurrency fixture 1 $30.00 $30.00"
        };
        var firstPdf = CreateMinimalTextPdf([.. commonLines, "Proof copy A"]);
        var secondPdf = CreateMinimalTextPdf([.. commonLines, "Proof copy B"]);
        ...
        try
        {
            ...
            var uploads = new[]
            {
                UploadAfterBarrierAsync(client, barrier.Task, firstFileName, firstPdf),
                UploadAfterBarrierAsync(concurrentClient, barrier.Task, secondFileName, secondPdf)
            };
            barrier.SetResult(true);
            ...
            AssertEqual(
                "1",
                results.Sum(result => result.GetProperty("invoiceDocumentCreated").GetInt32()).ToString(CultureInfo.InvariantCulture),
                "simultaneous invoice intake created count");
            AssertEqual(
                "1",
                results.Sum(result => result.GetProperty("invoiceDocumentLinked").GetInt32()).ToString(CultureInfo.InvariantCulture),
                "simultaneous invoice intake linked count");
            AssertEqual(
                "0",
                results.Sum(result => result.GetProperty("invoiceDocumentNeedsReview").GetInt32()).ToString(CultureInfo.InvariantCulture),
                "simultaneous invoice intake review count");

            var matches = (await GetJsonArrayAsync(client, "/api/invoice-documents?includeArchived=true"))
                .Where(document => string.Equals(
                    document.GetProperty("docNumber").GetString(),
                    documentNumber,
                    StringComparison.Ordinal))
                .ToArray();
            AssertEqual("1", matches.Length.ToString(CultureInfo.InvariantCulture), "simultaneous invoice intake record count");

            var proofs = (await GetJsonArrayAsync(client, "/api/audit-documents?includeArchived=true"))
                .Where(document => string.Equals(document.GetProperty("fileName").GetString(), firstFileName, StringComparison.Ordinal)
                    || string.Equals(document.GetProperty("fileName").GetString(), secondFileName, StringComparison.Ordinal))
                .ToArray();
            AssertEqual("2", proofs.Length.ToString(CultureInfo.InvariantCulture), "simultaneous invoice intake proof count");
            AssertTrue(
                proofs.All(document => !document.GetProperty("needsReview").GetBoolean()
                    && string.Equals(document.GetProperty("relatedRecordNumber").GetString(), documentNumber, StringComparison.Ordinal)),
                "simultaneous invoice intake links both proofs without leaving a false review item");
        }
        finally
        {
            await DeleteUploadedTestProofsAsync(client, root, [firstFileName, secondFileName]);
        }
    }

    static async Task<JsonElement> UploadAfterBarrierAsync(
        HttpClient client,
        Task barrier,
        string fileName,
        byte[] bytes)
    {
        await barrier;
        return await UploadDocumentAsync(client, fileName, bytes, "Invoice");
    }
    ...
    static async Task AssertSafeRouteSmokeMatrixAsync(HttpClient client)
    {
        ...
        var archivedObligation = generatedCalendar[0];
        var archivedObligationId = archivedObligation.GetProperty("id").GetInt32();
        var archivedObligationTitle = archivedObligation.GetProperty("title").GetString();
        var archivedObligationPeriod = archivedObligation.GetProperty("period").GetString();
        using (var archiveObligation = await client.DeleteAsync($"/api/tax-obligations/{archivedObligationId}"))
        {
            AssertStatus(HttpStatusCode.NoContent, archiveObligation.StatusCode, "archive generated tax obligation");
        }
        await PostJsonAsync(client, "/api/tax-calendar/generate?year=2098", new { });
        var regeneratedCalendar = await GetJsonArrayAsync(client, "/api/tax-calendar?year=2098");
        var replacementObligation = regeneratedCalendar.Single(item =>
            string.Equals(item.GetProperty("title").GetString(), archivedObligationTitle, StringComparison.OrdinalIgnoreCase)
            && string.Equals(item.GetProperty("period").GetString(), archivedObligationPeriod, StringComparison.OrdinalIgnoreCase));
        using (var blockedRestore = await client.PostAsJsonAsync($"/api/tax-obligations/{archivedObligationId}/restore", new { }))
        {
            AssertStatus(HttpStatusCode.Conflict, blockedRestore.StatusCode, "archived obligation restore rejects regenerated active identity");
            var conflict = await ReadJsonAsync(blockedRestore);
            AssertEqual(
                replacementObligation.GetProperty("id").GetInt32().ToString(CultureInfo.InvariantCulture),
                conflict.GetProperty("existingTaxObligationId").GetInt32().ToString(CultureInfo.InvariantCulture),
                "tax obligation restore conflict identifies regenerated active obligation");
        }
        ...
        var sourceFileAudit = await PostJsonAsync(client, "/api/audit-documents", new
        {
            documentDate = "2099-08-20",
            documentType = "Other",
            fileName = "Program.cs",
            filePathOrUrl = Path.Combine(FindWorkspaceRoot(), "Program.cs"),
            needsReview = true,
            notes = "Proof-root boundary fixture"
        });
        using (var sourceFileResponse = await client.GetAsync($"/api/audit-documents/{sourceFileAudit.GetProperty("id").GetInt32()}/file"))
        {
            AssertStatus(HttpStatusCode.BadRequest, sourceFileResponse.StatusCode, "source file outside proof roots");
        }
    ...
    static async Task AssertAiEstimateDraftCleansGmailPasteAsync(HttpClient client)
    {
        ...
        Skip to content Using Gmail with screen readers Enable desktop notifications for Gmail. OK No thanks 10 of 17 Estimate for Marine Control Bracket Inbox Sample Shop <sample-epata@example.test> Attachments Wed, May 20, 10:58 PM to jordan Hi Jordan, Attached is the estimate for your custom 3D print request. Please review the price, project details, material, and notes. If everything looks good, reply with approval and I'll move forward. If anything needs to be adjusted, send me the change.
        """;
        ...
        AssertContains("Marine Control Bracket", projectName, "Gmail cleanup project name");
        AssertContains("marine control bracket", projectDescription, "Gmail cleanup project description");
        AssertContains("Jordan", customerName, "Gmail cleanup customer name");
        ...
        AssertDoesNotContain("sample-epata@example.test", customerEmail, "Gmail cleanup customer email");
        AssertDoesNotContain("sample-epata@example.test", projectDescription, "Gmail cleanup project description");
    }

    static async Task AssertAiEstimateDraftExtractsDocumentPacketAsync(HttpClient client)
    {
        using var form = new MultipartFormDataContent();
        form.Add(new StringContent("WAF PDF and DOCX sources"), "sourceName");

        var docx = new ByteArrayContent(CreateMinimalDocxBytes());
        docx.Headers.ContentType = MediaTypeHeaderValue.Parse("application/vnd.openxmlformats-officedocument.wordprocessingml.document");
        form.Add(docx, "files", "customer-request.docx");

        var pdf = new ByteArrayContent(Encoding.Latin1.GetBytes("%PDF-1.4 BT (PDF custom sign request) Tj ET %%EOF"));
        pdf.Headers.ContentType = MediaTypeHeaderValue.Parse("application/pdf");
        form.Add(pdf, "files", "customer-reference.pdf");

        using var response = await client.PostAsync("/api/ai/estimate-draft/upload", form);

        AssertStatus(HttpStatusCode.OK, response.StatusCode, "AI estimate document packet extraction");
        var result = await ReadJsonAsync(response);
        var prefill = result.GetProperty("prefill");
        var lineItems = prefill.GetProperty("lineItems").EnumerateArray().ToArray();

        AssertTrue(!result.GetProperty("usedAi").GetBoolean(), "document packet extraction should use deterministic local rules");
        AssertEqual("LOCAL RULES", result.GetProperty("executionReceipt").GetProperty("engine").GetString(), "document packet extraction engine");
        AssertEqual("Acceptance DOCX Customer", prefill.GetProperty("customerName").GetString(), "DOCX customer mapping");
        AssertContains("DOCX bracket", prefill.GetProperty("projectName").GetString(), "DOCX project mapping");
        AssertContains("DOCX bracket", prefill.GetProperty("projectDescription").GetString(), "DOCX description mapping");
        AssertContains("PDF custom sign request", prefill.GetProperty("projectDescription").GetString(), "PDF description mapping");
        AssertDoesNotContain("SOURCE FILE", prefill.GetProperty("projectName").GetString(), "document packet project name");
        AssertDoesNotContain("SOURCE FILE", prefill.GetProperty("projectDescription").GetString(), "document packet project description");
        AssertTrue(lineItems.Any(item =>
                string.Equals(item.GetProperty("description").GetString(), "DOCX bracket", StringComparison.Ordinal)
                && item.GetProperty("quantity").GetDecimal() == 2m
                && item.GetProperty("rate").GetDecimal() == 31m),
            "DOCX item, quantity, and price should map to one line item");
    }

    static byte[] CreateMinimalDocxBytes()
    {
        ...
        using (var archive = new ZipArchive(stream, ZipArchiveMode.Create, true))
        {
            ...
            writer.Write("""
                ...
                <w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
                  <w:body>
                    <w:p><w:r><w:t>Customer: Acceptance DOCX Customer</w:t></w:r></w:p>
                    <w:p><w:r><w:t>- 2x DOCX bracket - $31</w:t></w:r></w:p>
                  </w:body>
                </w:document>
                """);
        }
...
sealed class EpataFactory(string root, string dbPath) : WebApplicationFactory<global::Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            services.AddHttpClient<InvoiceAppImportService>()
                .ConfigurePrimaryHttpMessageHandler(static () => new LegacyInvoiceAppHandler());
        });
    }
}

sealed class LegacyInvoiceAppHandler : HttpMessageHandler
{
    public const string DocumentNumber = "INV-2099-LEGACY-ARCHIVED";

    protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        var path = request.RequestUri?.AbsolutePath;
        var json = path?.Equals("/api/documents", StringComparison.OrdinalIgnoreCase) == true
            ? $$"""
                [{
                  "id": 1,
                  ...
                  "updatedAt": "2099-09-02T12:00:00Z"
                }]
                """
            : path?.Equals("/api/documents/1", StringComparison.OrdinalIgnoreCase) == true
                ? $$"""
                    {
                      "id": 1,
                      "docNumber": "{{DocumentNumber}}",
